...
# Automated release script for Multi Agent Config
...
TAG_VERSION=$(git describe --exact-match --tags HEAD | sed "s/multi-agent-config-v//")
if [ "$TAG_VERSION" != "$NEW_VERSION" ]; then
    log_error "Created tag version ($TAG_VERSION) doesn't match expected version ($NEW_VERSION)"
    exit 1
fi

log_success "Created tag: multi-agent-config-v$NEW_VERSION"

# Final confirmation
echo
log_info "Ready to push release:"
log_info "  Version: $NEW_VERSION"
log_info "  Tag: multi-agent-config-v$NEW_VERSION"
...
log_info "Monitor the release at: https://github.com/jfb/multi-agent-config/actions"
log_info "Release will be available at: https://github.com/jfb/multi-agent-config/releases/tag/multi-agent-config-v$NEW_VERSION"