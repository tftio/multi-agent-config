...
pub fn run_update(version: Option<&str>, force: bool, install_dir: Option<&Path>) -> i32 {
    ...
    match perform_update(&target_version, &install_path) {
        Ok(()) => {
            ...
            println!();
            println!("Run 'multi-agent-config --version' to verify the installation.");
            0
        }
        Err(e) => {
...
fn get_latest_version() -> Result<String, String> {
    let client = reqwest::blocking::Client::builder()
        .user_agent("multi-agent-config-updater")
...
fn perform_update(version: &str, install_path: &Path) -> Result<(), String> {
    ...
    let filename = format!("multi-agent-config-{platform}.{archive_ext}");
    ...
    let download_url = format!(
        "https://github.com/{}/releases/download/multi-agent-config-v{}/{}",
